...
async function fetchFavPets(userId) {
  try {
    const response = await fetch(`https://adoptapet-production-1bb7.up.railway.app:8080/api/Favorite?user=${userId}`);
    if (!response.ok) {
...
async function fetchUserInfo(userId) {
  try {
    const response = await fetch(`https://adoptapet-production-1bb7.up.railway.app:8080/api/UserAccounts/by-id/${userId}`);
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }
...
async function displayAppointments(userId) {
  ...
  const petProfilesResponse = await fetch(`https://adoptapet-production-1bb7.up.railway.app:8080/api/Pets`);
  const petProfiles = await petProfilesResponse.json();

  const appointmentsResponse = await fetch(`https://adoptapet-production-1bb7.up.railway.app:8080/api/Appointments/ByUser/${userId}?deleted=0`);  
  const appointments = await appointmentsResponse.json();

  const sheltersResponse = await fetch(`https://adoptapet-production-1bb7.up.railway.app:8080/api/Shelters`);
...
async function deleteAppointment(appointmentId) {
  try {
    const response = await fetch(`https://adoptapet-production-1bb7.up.railway.app:8080/api/Appointments/${appointmentId}`, {
      method: "DELETE",
      headers: {
